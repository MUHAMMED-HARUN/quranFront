...
import { Box, Paper, Typography, Button, CircularProgress, Alert, Table, TableBody, TableCell, TableContainer, TableHead, TableRow } from "@mui/material";
...
export const GroupStudentsList = () => {
    ...
    const { data: enrollmentsRes, isLoading, error } = useStudentEnrollmentsByGroupIdQuery(groupId || "");
    const enrollments: StudentEnrollment[] = Array.isArray(enrollmentsRes) ? enrollmentsRes : (enrollmentsRes as any)?.Value || [];

    const [isDailyEvaluationOpen, setIsDailyEvaluationOpen] = useState(false);
    const [selectedStudentForEval, setSelectedStudentForEval] = useState<StudentEnrollment | null>(null);

    const [isNominationOpen, setIsNominationOpen] = useState(false);
    const [selectedEnrollmentForNomination, setSelectedEnrollmentForNomination] = useState<StudentEnrollment | null>(null);
    ...
    return (
        <Box display="flex" flexDirection="column" gap={3}>
            ...
            <TableContainer component={Paper} sx={{ width: "100%", overflow: "hidden", p: 2 }}>
                {!enrollments.length ? (
                    <Alert severity="info">لا يوجد طلاب مسجلين في هذه المجموعة حالياً.</Alert>
                ) : (
                    <Table>
                        <TableHead>
                            <TableRow>
                        ...
                        <TableBody>
                            {enrollments.map((enrollment) => {
                                const canNominate = enrollment.HasInProgressDetails && !enrollment.HasPendingOrApprovedNominations;
                                return (
        <TableRow key={enrollment.StudentEnrollmentID || (enrollment as any).studentEnrollmentId} hover>
            <TableCell>{enrollment.StudentName || (enrollment as any).studentName}</TableCell>
            <TableCell>{enrollment.NationalNumber || (enrollment as any).nationalNumber}</TableCell>
            <TableCell>
                <Typography color={enrollment.IsActive !== undefined ? (enrollment.IsActive ? "success.main" : "error.main") : ((enrollment as any).isActive ? "success.main" : "error.main")} variant="body2">
                    {(enrollment.IsActive !== undefined ? enrollment.IsActive : (enrollment as any).isActive) ? "مفعل" : "غير مفعل"}
                </Typography>
            </TableCell>
                                        <TableCell>
                                            <Box display="flex" gap={1}>
                                                <Button
                                                    ...
                                                    disabled={!(enrollment.IsActive !== undefined ? enrollment.IsActive : (enrollment as any).isActive)}
                                                >
                                                    إضافة تسميع
                                                </Button>
